...
export default function ModalEditarProducto({ producto, onClose, onSuccess }) {
  ...
  const [qr, setQr] = useState(producto.codigo_qr || "");
  const [estatus, setEstatus] = useState(producto.estatus || "Activo");
  ...
  const handleGuardar = async (e) => {
    e.preventDefault();
    if (!nombre.trim()) {
      alertaToast("error", "⚠️ El nombre es obligatorio.");
      return;
    }

    setGuardando(true);
    try {
      const res = await fetch(`${API_URL}/editar_producto.php`, {
        method: "POST",
        headers: {
    ...
    <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-[999] flex items-center justify-center p-4 animate-fade-in-up">
      <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 w-full max-w-md shadow-2xl border border-slate-200 dark:border-slate-700">
        <h3 className="text-xl font-black text-slate-800 dark:text-white border-b border-slate-100 dark:border-slate-700 pb-3 mb-4 flex items-center gap-2">
          <span>✏️</span> Editar Producto #{producto.id}
        </h3>
        ...
        <form onSubmit={handleGuardar} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-white dark:bg-slate-900/50 border border-slate-300 dark:border-slate-600 rounded-xl p-2.5 text-sm outline-none font-bold text-slate-800 dark:text-white"
              >
                <option value="Activo">● Activo</option>
                <option value="Inactivo">🚫 Inactivo (Desactivar)</option>
              </select>
            </div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-50 dark:bg-slate-900/50 border border-slate-300 dark:border-slate-600 rounded-xl p-2.5 text-sm outline-none font-mono text-slate-800 dark:text-white"
            />
          </div>
